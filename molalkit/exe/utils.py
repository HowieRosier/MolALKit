...
def get_model_from_config(model_config: Dict, dataset, task_type, save_dir,
    ...
    return get_model(
        ...
        maturity_threshold=model_config.get("maturity_threshold", maturity_threshold) if maturity_threshold is not None else model_config.get("maturity_threshold", 100),
        replacement_rate=model_config.get("replacement_rate", replacement_rate) if replacement_rate is not None else model_config.get("replacement_rate", 0.001),
        decay_rate=model_config.get("decay_rate", decay_rate) if decay_rate is not None else model_config.get("decay_rate", 0.95),
